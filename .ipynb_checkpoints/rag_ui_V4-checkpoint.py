...
def main():
    ...
    if feature == "Chatbot":
        ...
        st.title("Summarization dari Dokumen")
        st.write("Unggah file (.txt, .pdf, .docx) untuk diringkas.")

        uploaded_file = st.file_uploader("Pilih file:", type=["txt", "pdf", "docx"])
        if uploaded_file:
            if uploaded_file.name.endswith(".txt"):
                paragraphs = parse_txt(uploaded_file)
            elif uploaded_file.name.endswith(".pdf"):
                paragraphs = parse_pdf(uploaded_file)
            elif uploaded_file.name.endswith(".docx"):
                paragraphs = parse_docx(uploaded_file)
